...
def test_failure_display_name_falls_back_to_root_cause_when_unknown():
    """UNKNOWN is a real classify_failure bucket, not missing data -- but as
    a graph node label it tells the viewer nothing, so it should fall back
    to the actual root cause the same way a missing category would."""
