"""Tests for the Peer Review Agent's deterministic node (detect_workflow_changes).

review_diff is a Bedrock call and isn't unit-tested here without mocking the
client — mirrors test_root_cause_node.py's scope (schema/deterministic paths
only, not the LLM call itself).
"""
import os
