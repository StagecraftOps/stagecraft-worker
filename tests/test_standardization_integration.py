"""Integration test: pattern-frequency detection against the real
pace-stagecraft-monorepo fixture's 82 per-service ci-*.yml files, which the
repo's own docs describe as sharing a common 5-stage template-call pattern.
Skipped if the sibling fixture repo isn't checked out locally.
"""
import os
...
def test_shared_template_pattern_detected_across_real_per_service_files():
    ...
    # Every real per-service file calls the shared _template-*.yml reusable
    # workflows — the top cluster should reflect that widespread reuse.
